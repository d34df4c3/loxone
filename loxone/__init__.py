...
@frappe.whitelist()
def load_users(ms_name: str) -> list[dict]:
    ...
    try:
        ms_doc = Miniserver.get_doc(ms_name)
...
@frappe.whitelist()
def load_user_groups(ms_name: str) -> list[dict]:
...
def save_user_keycode(ms_name: str, user_name: str, keycode: str) -> None:
    ...
    # Update Miniserver
    from loxone.loxone.doctype.miniserver.miniserver import Miniserver
    updated = Miniserver.get_doc(ms_name).get_miniserver().set_keycode(user_doc.lx_uuid, keycode)
    if not updated:
